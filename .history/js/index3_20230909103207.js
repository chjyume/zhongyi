  ...
  if (currentTextIndex < texts.length - 1) {
    ...
    var typing = setInterval(function() {
      if (i < currentText.length) {
        ...
        i++;
        if (currentTextIndex === 4 || currentTextIndex === 5) {
          overlay.style.display = 'flex'; // 显示遮罩层
          textDiv.style.display = 'none'; // 隐藏textDiv
          endMarker.style.display = 'none';
          overlay.addEventListener('click', hideOverlay); // 添加点击遮罩层的事件监听器
        }
...
function hideOverlay() {
  ...
  endMarker.style.display = 'block';
  overlay.removeEventListener('click', hideOverlay);// 移除点击遮罩层的事件监听器
  showNextText(); // 加载下一段文字
}
